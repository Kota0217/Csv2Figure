...
#データ読み出し
with open('data/kadai2/MullerLyer_KI_001_pf.csv') as f:
    reader = csv.reader(f, quoting=csv.QUOTE_NONNUMERIC)
    data_1 = [row for row in reader]


with open('data/kadai2/MullerLyer_KI_002_pf.csv') as f:
    reader = csv.reader(f, quoting=csv.QUOTE_NONNUMERIC)
    data_2 = [row for row in reader]

with open('data/kadai2/MullerLyer_KI_003_pf.csv') as f:
    reader = csv.reader(f, quoting=csv.QUOTE_NONNUMERIC)
...
plt.xlabel("Inward - Outward length (Degree = 30[degree])")
